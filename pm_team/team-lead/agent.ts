/**
 * Team Lead Agent — Task Runner
 *
 * Picks up tasks from agent_tasks table targeted at 'team-lead',
 * routes to the correct command module, writes results back.
 */

import type { TeamLeadCommand } from './lib/types.js'
...
function parseCommand(description: string): TeamLeadCommand | null {
  // Try JSON first
  try {
    const parsed = JSON.parse(description)
    if (parsed.type) return parsed as TeamLeadCommand
  } catch {
  ...
  if (lower.includes('hygiene') || lower.includes('backlog')) {
    return { type: 'hygiene' }
  }
  if (lower.includes('synthe')) {
    return { type: 'synthesize' }
  }
  if (lower.includes('enforce') || lower.includes('compliance')) {
    return { type: 'enforce' }
  }
...
async function executeCommand(command: TeamLeadCommand): Promise<{ summary: string; details?: Record<string, unknown> }> {
  switch (command.type) {
    case 'hygiene': {
    ...
    case 'synthesize': {
      ...
      const result = await run({ days: command.days, agents: command.agents })
      return { summary: result.summary, details: result.details }
    }
    case 'enforce': {
      const { run } = await import('./commands/enforce.js')
      const result = await run({ workflow: command.workflow })
      return { summary: result.summary, details: { stats: result.stats } }
    }
  }
}
...
export async function checkTasks(): Promise<{ processed: number; errors: number }> {
  ...
  if (pending.length === 0) {
    console.log('No pending tasks for team-lead.')
  ...
  for (const task of pending) {
    ...
    await supabase
      .from('agent_tasks' as any)
      .update({ status: 'picked-up', picked_up_by: 'team-lead' } as any)
      .eq('id', task.id)
    ...
    try {
      ...
      await logError('team-lead', err instanceof Error ? err : new Error(message), {
        task_id: task.id,
