...
2. **Read your own availability.** `schedule:busy --window=YYYY-MM-DD..YYYY-MM-DD` (local osascript; your calendar only). Attendee *free/busy* isn't available locally — proposed times are based on your openings; but attendee **timezone + work week ARE** honored (see step 3), so cross-zone people get sane slots; invitees still accept or counter.
3. **Rank times (timezone-aware).** Feed busy blocks into `schedule:find-times --payload=<{windowStartDay,windowEndDay,durationMin,busy,nowNaive,count, organizerTz?, attendees?}>`. Your defaults: 30-min, Sun–Thu, 09:00–18:00, no 13:00 lunch, 15-min buffer. **Pass `attendees:[{timezone}]`** (from step 1's `resolve` output — each carries `timezone`/`location`) so a slot is kept only when it's inside **every** attendee's local work window (work hours + work week: Israel Sun–Thu, elsewhere Mon–Fri). E.g. a New York attendee gets Israel-evening = NY-morning slots and no Sunday. `organizerTz` defaults to `Asia/Jerusalem`. Use `formatInZone` (`schedule/tz.ts`) to show each slot in the attendee's zone in the chat slate. Pass `count` = meetings to spread one-per-day.
